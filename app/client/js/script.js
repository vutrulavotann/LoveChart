(function($) {
    $.fn.digita_stickyMenu = function(options) {
        var $this = this, defaults = {'left': 0,'top': 0,'menu_offset_top': $this.offset().top}, settings = $.extend({}, defaults, options);
...
jQuery(document).ready(function($) {
    ...
    $(".tallykit-shortcode-alert-close").click(function() {
        $(this).parent(".tallykit-shortcode-alert-block").hide("slow");
    });
    $(".tallykit-shortcode-tabs").tabs({fx: {opacity: 'toggle'},select: function(event, ui) {
    ...
    $("h3.tallykit-shortcode-toggle-trigger").click(function() {
    ...
    if (jQuery().waypoint) {
        jQuery('.tallykit-shortcode-progressBar').waypoint(function() {
            ...
            jQuery(this).find('.tallykit-shortcode-progressBar-content').animate({width: percentage + '%'}, 'slow');
        }, {triggerOnce: true,offset: '100%'});
    }
    $('.tallykit_single_gallery').magnificPopup({delegate: '.magnificPopup-child',type: 'image',gallery: {enabled: true,navigateByImgClick: true,preload: [0, 1]},});
    $("#rsvpForm").validate({submitHandler: function(form) {
            $.ajax({type: 'POST',url: 'php/sendrsvp.php',data: $("#rsvpForm").serialize(),beforeSend: function() {
                    $("#rsvpForm").hide();
                    $(".sending").show();
                },success: function() {
                    $(".sending").hide();
